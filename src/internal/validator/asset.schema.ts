...
export const CreateAssetRequest = z.object(
  {
    ...
    assetID: z
      .array(
        ...
        "ต้องการลิสต์ของเลขครุภัณฑ์",
      )
      .nonempty("กรุณากรอกเลขครุภัณฑ์อย่างน้อย 1 รายการ"),
  },
  {
    error: (issue) => {
...
export const DeleteAssetRequest = z.object(
  {
    id: z.coerce
      .number({
        error: (issue) =>
          issue.input === undefined
            ? "กรุณากรอกไอดีของครุภัณฑ์"
            : "ไอดีครุภัณฑ์ต้องเป็นตัวเลข",
      })
      .min(1, "ไอดีครุภัณฑ์ต้องมากกว่า 0")
      .max(2147483647, "ไม่พบครุภัณฑ์ดังกล่าว")
      .int("ไอดีครุภัณฑ์ต้องเป็นจำนวนเต็ม"),
  },
  {
    error: (issue) => {
      if (issue.input === undefined) {
